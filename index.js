...
function findElementGreaterThan(arr, num) {
  ...
  // Перевіряємо, чи знайдено елемент, який задовольняє умову.
  if (foundElement !== undefined) {
    // Якщо так, повертаємо цей елемент.
    return foundElement;
  } else {
    // Якщо не знайдено жодного елемента, що задовольняє умову, повертаємо рядок "Числа, яке задовольняє умову, не знайдено".
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const lastNEllements = arr.slice(-n);

  const sum = lastNEllements.reduceRight(
    (accumulator, currentElement) => accumulator + currentElement
  );

  lastNEllements.forEach((element) => console.log(element));
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((prevLongest, currentWord) => {
    if (currentWord.length > prevLongest.length) {
      return currentWord;
    } else {
      return prevLongest;
    }
